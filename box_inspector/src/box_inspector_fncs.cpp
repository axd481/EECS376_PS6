...
bool BoxInspector::model_poses_wrt_box(osrf_gear::Shipment &shipment_status) {
    ...
    for (int imodel = 0; imodel < num_models; imodel++) {
        if (imodel != i_box) { //if here, have a model NOT the box
            ...
            // get pose for each product with respect to the world
            // get pose of the box with respect to the world
            //transform from respect to world to respect to box
            //pushback, which happens below
...
void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
   ...
   //now must create poses that will then be transformed from box to world
   //create loop that creates product for each part
   // for (int i = 0; i < num_parts_seen; i++){
   //add in this for loop designating poses for each product 
//}
   //we would then transform those poses with respect to the box
    //then transform poses from respect to the box to the world.
    // then pushback the poses
    //shipment_status.products.push_back(product);
