...
// orphan_models_wrt_world: vector of models that are seen in the box, but DO NOT belong in the box
  ...
  //go through the parts and add all of the good ones to the list
    //if (box_inspector_image_models[i]
    //Above if will check if the parts on the list are within the tolerance
    //if they are then add them to a list of good parts using a for loop with number of good parts
    ////then in the same for loop check if the parts are missing and then add those to a list
    //check again for extra parts
    //pushback these lists to update inspection.
//
//}
